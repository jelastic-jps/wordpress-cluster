...
class hyperdb extends wpdb {
	...
	function query( $query ) {
		...
		if ( $this->last_error = $this->ex_mysql_error( $this->dbh ) ) {
			$this->dbhname_heartbeats[$this->dbhname]['last_errno'] = $this->ex_mysql_errno( $this->dbh );
	...
	function mark_server_as_down( $host, $port, $apc_ttl = 10 ) {
		if ( ! function_exists( 'apc_store' ) )
			return;

		apc_add( "server_state_$host$port", 'down', $apc_ttl );
	}
